...
class Analytics(object):
    ...
    def initialise(timestamps: List[pd.Timestamp], fills: List[TradeFill], prices: np.ndarray, predictions: Optional[Dict[Timestamp, np.ndarray]]) -> Analytics:
        ...
        if bool(predictions):
            ...
            mda = np.ma.mean((exp_ret >= 0) == (real_ret >= 0), axis=0)
            correlation = [None if np.isnan(np.ma.corrcoef(x, y)[1,0]) else np.ma.corrcoef(x, y)[1,0] for x, y in zip(exp_ret.T, real_ret.T)]
