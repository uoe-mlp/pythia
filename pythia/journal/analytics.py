...
class Analytics(object):
    ...
    def __init__(self, timeseries: pd.Series, volatility: float, cumulative_return: float, sharpe_ratio: float, sortino_ratio: float, maximum_drawdown: float,
        ...
        training_mda: Optional[np.ndarray], training_corr: Optional[np.ndarray]):
        ...
        self.training_mda: np.ndarray = training_mda if training_mda is not None else None
        self.training_corr: np.ndarray = training_corr if training_corr is not None else None

    @staticmethod
    def initialise(timestamps: List[pd.Timestamp], fills: List[TradeFill], prices: np.ndarray,
    price_predictions: Optional[Dict[Timestamp, np.ndarray]], instruments: List[str], training_predictions: Optional[pd.DataFrame]=None,
    training_mda=None, training_corr=None) -> Analytics:
        ...
        if bool(price_predictions):
            p_df = pd.DataFrame(price_predictions).transpose()
        ...
        return Analytics(
            ...
            training_mda=training_mda if training_mda is not None else None,
            training_corr=training_corr if training_corr is not None else None)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        data['training_correlation'] = self.training_corr.T.tolist() if self.training_corr is not None else None
        data['training_mda'] = self.training_mda.T.tolist() if self.training_mda is not None else None
        data['predictions'] = {
            name: {
                'values': series.to_list(), 
                'dates': [x.strftime('%Y-%m-%d') for x in series.index.tolist()]}
            for name, series in self.price_predictions.iteritems()} if self.price_predictions is not None else None
